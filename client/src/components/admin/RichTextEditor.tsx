    ...
    >
      <div className="flex flex-wrap items-center gap-1 border-b border-base-300 bg-base-200 p-1">
        ...
        <div className="dropdown dropdown-hover">
          <label tabIndex={0} className="btn btn-sm btn-ghost cursor-pointer" aria-label="Text color">
            A
            <span
              className="ml-0.5 inline-block h-3 w-3 rounded border border-base-content/30"
              style={{
                backgroundColor:
                  (editor.getAttributes("textStyle") as { color?: string }).color ?? "currentColor",
              }}
            />
          </label>
          <ul
        ...
        {editor.isActive("bulletList") && (
          <select
            ...
            value={
              (editor.getAttributes("bulletList").listStyleType as string) || "disc"
            }
            onChange={(e) => {
      ...
      />
      <style jsx global>{`
        .rich-text-editor .ProseMirror p { margin: 0.25em 0; }
        .rich-text-editor .ProseMirror h2 { font-size: 1.25rem; font-weight: 700; margin: 0.5em 0; }
        .rich-text-editor .ProseMirror h3 { font-size: 1.1rem; font-weight: 600; margin: 0.4em 0; }
        .rich-text-editor .ProseMirror a { color: hsl(var(--p)); text-decoration: underline; }
        .rich-text-editor .ProseMirror ul, .rich-text-editor .ProseMirror ol { margin: 0.5em 0; padding-left: 1.5rem; }
        .rich-text-editor .ProseMirror li { margin: 0.2em 0; }
      `}</style>
    </div>
  );
}
