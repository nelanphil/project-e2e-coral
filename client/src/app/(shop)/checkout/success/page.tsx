...
export default function CheckoutSuccessPage() {
  ...
  const orderTotal = order?.lineItems.reduce((sum, item) => sum + item.price * item.quantity, 0) ?? 0;

  return (
    <main className="container mx-auto px-4 py-12 max-w-2xl">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold">Order confirmed</h1>
        <p className="mt-2 text-base-content/70">We&apos;ll ship your coral soon.</p>
      </div>
      ...
      {!loading && !order && (
        <div className="text-center">
          <p className="text-base-content/60 mb-6">Your order was placed successfully.</p>
              ...
              <tbody>
                {order.lineItems.map((item, i) => (
                  <tr key={i} className="border-b border-base-300 last:border-0">
                    ...
                    <td className="py-2 text-right">${(item.price / 100).toFixed(2)}</td>
                    <td className="py-2 text-right">${((item.price * item.quantity) / 100).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
