 ...
 */
function batchesGet(client, batchId, page, results, options) {
    return new async_js_1.APIPromise($do(client, batchId, page, results, options));
}
async function $do(client, batchId, page, results, options) {
    const input = {
        batchId: batchId,
        page: page,
        results: results,
    };
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => z.parse(operations.GetBatchRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
