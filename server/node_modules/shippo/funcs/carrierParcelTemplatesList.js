 ...
 * </ul>
 */
function carrierParcelTemplatesList(client, include, carrier, options) {
    return new async_js_1.APIPromise($do(client, include, carrier, options));
}
async function $do(client, include, carrier, options) {
    const input = {
        include: include,
        carrier: carrier,
    };
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => z.parse(operations.ListCarrierParcelTemplatesRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(200, components.CarrierParcelTemplateList$inboundSchema), M.fail([400, "4XX"]), M.fail("5XX"))(response, req);
    if (!result.ok) {
