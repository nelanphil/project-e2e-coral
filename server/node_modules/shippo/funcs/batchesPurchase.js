 ...
 */
function batchesPurchase(client, batchId, options) {
    return new async_js_1.APIPromise($do(client, batchId, options));
}
async function $do(client, batchId, options) {
    const input = {
        batchId: batchId,
    };
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => z.parse(operations.PurchaseBatchRequest$outboundSchema, value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(202, components.Batch$inboundSchema), M.fail([400, "4XX"]), M.fail("5XX"))(response, req);
    if (!result.ok) {
